...
const Header = () => {
  ...
  return (
    <section className="flex w-full max-w-8xl flex-row items-center justify-between p-5 lg:px-10%">
      ...
      <div className="hidden md:block">
        <ul className="flex gap-5 lg:gap-14 hover:[&_a]:text-primary-one">
          <li><Link href="/" rel="noreferrer">Home</Link></li>
          <li><Link href="/" rel="noreferrer">New</Link></li>
          <li><Link href="/" rel="noreferrer">Popular</Link></li>
          <li><Link href="/" rel="noreferrer">Trending</Link></li>
          <li><Link href="/" rel="noreferrer">Categories</Link></li>
        </ul>
      </div>
      <div className="drawer drawer-end w-fit md:hidden">
        ...
        <div className="drawer-content z-50">
          {/* Page content here */}
          <label htmlFor="my-drawer" className="text-4xl" onClick={() => toggleClick(!isClick)} aria-hidden="true">
            <div className={`${!isClick ? 'active:animate-rotate-menu' : 'active:animate-rotate-menu-back '}`}>{!isClick ? <GiHamburgerMenu /> : <GrClose /> }</div>
          </label>
        </div>
        <div className="drawer-side z-10">
          <label htmlFor="my-drawer" aria-label="close sidebar" className="drawer-overlay " />
          <ul className="menu min-h-full w-60 bg-neutral-one p-4 pt-28 [&_a]:text-neutral-four text-lg font-normal">
